...
MVPでは「抽出された翻訳候補文章の件数に対する割合」と定義します。

例:

- 候補が100文章、設定が20%なら20文章
- 候補が7文章、設定が20%なら1文章
- 0%ならAPIを呼ばず、何も翻訳しない
- n%が0より大きく候補が存在する場合は、最低1文章を対象とする
- 対象数は拡張機能側で計算し、APIへ `target_count` として渡す
- モデルに割合計算をさせない
...
ページが大きい場合は、文章の途中で分割せず、複数リクエストへ分割してください。ページ全体の最終的な翻訳文章数が `target_count` になるよう、各チャンクへ対象数を配分してください。
